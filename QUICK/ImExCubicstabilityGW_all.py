...
def alphaSafety_cNmax(c, Ndt):
    return max(0.5, 1-1/max(c*(1 + Ndt), eps))
...
plt.plot(cs, absA[0,:, 1,1], 'b-', label=r'$\beta=1$, $\gamma=1$, $\alpha< 1-1/c$')
plt.plot(cs, absA[0,:, 1,0], 'b--', label=r'$\beta=1$, $\gamma=0$, $\alpha< 1-1/c$')
plt.xlim(0,4)
plt.ylim(0.99,1.01)
plt.axhline(y=1, ls=(2,(2,10)), color='k', lw=0.25)
plt.axvline(x=0.88, ls=(2,(2,10)), color='k', lw=0.25)
...
plt.tight_layout()
fileName='Acubic_N0_gamma_1.pdf'
...
absA = calcAllMagA(Ndts, cs, betas, gammas, alpha_cN)
absA05 = calcAllMagA(Ndts, cs, betas, gammas, alpha_05)
absA_c = calcAllMagA(Ndts, cs, betas, gammas, alpha_c)
absAmax = calcAllMagA(Ndts, cs, betas, gammas, alphaSafety_cNmax)

for (A, fileName) in zip([absA, absA05, absA_c, absAmax], 
                  ['Acubic_gamma_1_beta_1.pdf',
                   'Acubic_gamma_1_beta_1_alpha05.pdf',
